...
extern "C" {
		...
		typedef struct GnssInsSystem
		{
			...
			uint16_t GNSSQUALITYMode; // ins system judge flag 
			uint16_t PreGNSSQUALITYMode; // ins system judge flag 
			
			int8_t useNativeGNSSPOS; //默认为0
			...
			int INSPROCESSMODE;// 1 INS MECHINE 2 EKF FEED BACK 3 OBS UPDATA NO FEED BACK
			int8_t GNSSZuptFlag;
			...
			MISRVB outputRVB; // when cal and  then save to file,output to config file ,current mis file
			...
			int32_t insinitfromprev;

			int8_t systemtime_ready;

            uint32_t system_init_status;    // ensure ins init quality
            uint32_t system_init_gnssbadflag;    
            uint32_t system_init_gnssgoodflag;


			int32_t initmode; //0:gnss fixed 1:float 2:spp
			double initspeed;// 
		}GnssInsSystem;
