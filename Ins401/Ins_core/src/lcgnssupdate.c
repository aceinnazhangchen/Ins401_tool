...
static int8_t MotionAlign(const GnssData* premGnssData, const GnssData* mGnssData, uint8_t GnssType, Nav *mNav)
{
	...
	// double vehchile_speed_M = 4.0;//p_prcopt.imuSensorType == 2 ? 10 : 5;
	// double vehchile_speed_M = 0;//p_prcopt.imuSensorType == 2 ? 10 : 5;
	// double vehchile_speed_M = 2;//p_prcopt.imuSensorType == 2 ? 10 : 5;
	double vehchile_speed_M = mGnssInsSystem.initspeed;//p_prcopt.imuSensorType == 2 ? 10 : 5;
	//double vehchile_speed_M = p_prcopt.imuSensorType == 2 ? 10 : 2;

	double dt = mGnssData->timestamp - premGnssData->timestamp;
	if (premGnssData->timestamp < 0) return 0;
	if (1 == GnssType)
	{
		if (mGnssData->gpsFixType >= 4 && premGnssData->gpsFixType >= 4 && dt < 2.0 && dt > 0.4)
		{
	...
	if (3 == GnssType && p_prcopt.gnssDataRate != 10) // 我们的GnssType为3， gnssDataRate为1
	{
		if (mGnssData->gpsFixType >= 4 && premGnssData->gpsFixType >= 4 && dt < 2.0 && dt > 0.4) // 保证相邻数据的时间为1秒且至少是float
		{
			Vel_NED[0] = mGnssData->north_velocity;
			Vel_NED[1] = mGnssData->east_velocity;
			Vel_NED[2] = -mGnssData->up_velocity;
			vehchile_speed = sqrt(Vel_NED[0] * Vel_NED[0] + Vel_NED[1] * Vel_NED[1]); //当前的水平速度
		}
		else
		{
			lastflag = 0;
	...
	else if (3 == GnssType && p_prcopt.gnssDataRate == 10)
	{
		if (mGnssData->gpsFixType >= 4 && premGnssData->gpsFixType >= 4)
		{
			Vel_NED[0] = mGnssData->north_velocity;
			Vel_NED[1] = mGnssData->east_velocity;
			Vel_NED[2] = -mGnssData->up_velocity;
	...
	int8_t is_line_motion = IsOnLine(); // 通过IMU数据判断直行
	if (vehchile_speed > vehchile_speed_M && is_line_motion) // 满足速度和直行条件
	{
		if (lastflag >= 1 && (mGnssData->gpsFixType == 4 || p_prcopt.gnssDataRate == 10)) // 需要当前fix
			//	|| lastflag >= 3 && (mGnssData->Mode >= 4 || p_prcopt.gnssDataRate == 10))
		{
			mNav->lon = mGnssData->longitude; // 赋nav的位置速度姿态
			...
			mNav->roll = 0.0;
			mNav->pitch = 0.0;
			double heading = atan2(mNav->ve, mNav->vn); // 根据速度计算航向
			if (heading < 0)
			{
				heading += 2 * PI;
			}
...
static int8_t ekf_measurement_update(float *x, float* P, const float *H, const int *L, float z, float R, int n, int m)
{
    ...
    // P = (I - K*H)*P = (I - PHt/(R+HPHt)*H)*P = (I - PHtH/P_inov)*P = P - PHtHP/P_inov
    //                                                                = P - PHt*(PHt)t/P_inov
	for (i = 0; i < n; ++i) 
	{
		for (j = 0; j < n; ++j)
	...
	for (i = 0; i < n; ++i) // 非对角线取平均后填入，使P阵对称
	{
		for (j = 0; j < i; ++j)
		{
...
static int8_t SetGNSSPOSMeasZ(const GnssData *mGnssData, float* Z)
{
	...
	Z[0] = (float)Z1[0]; // n系下，LC ins和gnss位置差
	Z[1] = (float)Z1[1];
	Z[2] = (float)Z1[2];
	ret = 1;
	return ret;
...
static int8_t Odoupdate()
{
	...
	double Vel_NED[3] = { mUpdataStruct.mPVA.northVelocity, mUpdataStruct.mPVA.eastVelocity,  mUpdataStruct.mPVA.downVelocity };
	double INS_Vv_Odo[3];
	MatrixAdd(Vel_NED, temp1, 3, 1, Vel_NED); //Vel_NED INS_Vn_Odo odo杆臂补偿速度

	double  C_vn[3][3], C_nv[3][3], C_bv[3][3];

	memcpy(C_vn, eye33, 9 * sizeof(double)); // 认为 b系和v系没误差
	memcpy(C_nv, eye33, 9 * sizeof(double));
	memcpy(C_bv, eye33, 9 * sizeof(double));
	...
	MatrixMutiply(*C_nv, Vel_NED, 3, 3, 1, INS_Vv_Odo); // 得到v系下的车速
	double v = mGnssInsSystem.mOdoData.vehicle_speed * mGnssInsSystem.mNav.Odo_scale * p_prcopt.odoScale; // 里程计速度
	...
	NHCmeasZ[0] = INS_Vv_Odo[0] - v; // 得到速度误差
...
int OdoObsUpdata(double systemtime)
{
	...
	if (p_prcopt.isUseNHC)
	{
		...
		{
			ret = 1;
			mGnssInsSystem.PerMeasUpdata = mGnssInsSystem.PerMeasUpdata | OdoUpdate;
			mGnssInsSystem.PerMeasUpdata = mGnssInsSystem.PerMeasUpdata | NHCUpdate;
		}
		else if (status == -2)
		{
...
int VirtualObsUpdata(double systemtime)
{
	...
	if (p_prcopt.isUseNHC & !p_prcopt.isUseNHCRES)
	{
...
int8_t ObsUpdata(double systemtime, GnssData mGnssData)
{
	...
	{
		if (mGnssData.timestamp - mGnssInsSystem.lastObstime < 0.8 / p_prcopt.useGNSSRate)
		{
	...
	/*GNSS中断 判定隧道*/
	int8_t Outstatus = 0;
	if (mGnssInsSystem.GNSSQUALITYMode != 0) // 有效
	{
		if ((mGnssInsSystem.premGnssData.timestamp > 0 && mGnssData.timestamp - mGnssInsSystem.premGnssData.timestamp > 60)) // 超过60s中断
		{
			mGnssInsSystem.OutOftunnalTime = mGnssData.timestamp; // 出隧道的时间
		}
		if (mGnssInsSystem.OutOftunnalTime > 0.0001)
		{
			if (mGnssData.timestamp - mGnssInsSystem.OutOftunnalTime < 10) // 出隧道10s内
			{
				Outstatus = 1;
			}
			else if (mGnssData.timestamp - mGnssInsSystem.OutOftunnalTime >= 10
				&& mGnssData.timestamp - mGnssInsSystem.OutOftunnalTime < 60) // 出隧道10s到60s
			{
				Outstatus = 0;
				mGnssInsSystem.OutOftunnalTime = 0;
			}
			else if (mGnssData.timestamp - mGnssInsSystem.OutOftunnalTime >= 60) // 出隧道大于60s
			{
				Outstatus = 0;
				mGnssInsSystem.OutOftunnalTime = 0;
			}
	...
	if (mGnssInsSystem.GNSSQUALITYMode == 5)
	{
		if (mGnssInsSystem.premGnssData.gpsFixType < 4)
		{
			mGnssInsSystem.GNSSQUALITYMode = 0;
	...
	if (mGnssInsSystem.GNSSQUALITYMode == 1)
	{
		if (mGnssInsSystem.premGnssData.gpsFixType != 1)
		{
			mGnssInsSystem.GNSSQUALITYMode = 0;
			mGnssInsSystem.GNSSREJRCTType = SolutionSataChange;
		}
	...
	if (p_prcopt.isUseHeadOnline && mGnssData.timestamp - mGnssInsSystem.firstGNSSUseTime > 120
		&& mGnssInsSystem.firstGNSSUseTime > 0) // isUseHeadOnline为0，不执行
	{
		{
	...
	float GNSSPOSmeasR[3] = { 0.0 }; //posstd
	float GNSSVELMeasZ[3] = { 0.0 };
	float GNSSVELmeasR[3] = { 0.0 };
	if (mGnssInsSystem.GNSSQUALITYMode != 0)
	{
		if (!mGnssInsSystem.useNativeGNSSPOS) // 为0
		{
			GNSSPOSmeasR[0] = p_prcopt.GNSSScale*mGnssData.latitude_std * mGnssData.latitude_std;
			GNSSPOSmeasR[1] = p_prcopt.GNSSScale*mGnssData.longitude_std * mGnssData.longitude_std;
			GNSSPOSmeasR[2] = p_prcopt.GNSSScale  * mGnssData.altitude_std * mGnssData.altitude_std; //4

			if (4 == mGnssInsSystem.GNSSQUALITYMode || 5 == mGnssInsSystem.GNSSQUALITYMode)
			{
			...
			else if (1 == mGnssInsSystem.GNSSQUALITYMode)
			{
				double scale = 1.0; // 0.25
				GNSSPOSmeasR[0] = scale * p_prcopt.GNSSScale * mGnssData.latitude_std * mGnssData.latitude_std;
				GNSSPOSmeasR[1] = scale * p_prcopt.GNSSScale * mGnssData.longitude_std * mGnssData.longitude_std;
				GNSSPOSmeasR[2] = scale * p_prcopt.GNSSScale * mGnssData.altitude_std * mGnssData.altitude_std;
			}


			if (mGnssInsSystem.GNSSQUALITYMode == 4)
			{
			...
			else if (mGnssInsSystem.GNSSQUALITYMode == 5)
			{
			...
			else if (mGnssInsSystem.GNSSQUALITYMode == 2)
			{
			...
			else if (mGnssInsSystem.GNSSQUALITYMode == 1)
			{
		...
		{
			GNSSPOSmeasR[0] = 0.09 * p_prcopt.GNSSScale*mGnssData.latitude_std * mGnssData.latitude_std;
			GNSSPOSmeasR[1] = 0.09 * p_prcopt.GNSSScale*mGnssData.longitude_std * mGnssData.longitude_std;
			GNSSPOSmeasR[2] = 0.09 * p_prcopt.GNSSScale* mGnssData.altitude_std * mGnssData.altitude_std;
		}
		if (p_prcopt.gnssSensorType != 0) // 为0
		{
			if (GNSSPOSmeasR[0] < 0.0122 *  p_prcopt.GNSSScale
				&& GNSSPOSmeasR[1] < 0.0122 *  p_prcopt.GNSSScale
				&& GNSSPOSmeasR[2] < 0.0122 *  p_prcopt.GNSSScale)
			{
	...
	if (!mGnssInsSystem.useNativeGNSSPOS && p_prcopt.isUseGNSSVel)
	{
		...
		if (mGnssData.Mode != 0)
		{
			if ((GNSSPOSmeasR[0] > 0.09 *  p_prcopt.GNSSScale|| GNSSPOSmeasR[1] > 0.09  *  p_prcopt.GNSSScale || GNSSPOSmeasR[2] > 0.09 *  p_prcopt.GNSSScale)
				&& gnss_speed < 2.0)
			{
				mGnssInsSystem.GNSSflag = 0;
			}
	...
	if (mGnssInsSystem.GNSSQUALITYMode != 0)
	{
		...
		if (p_prcopt.isUseExpQC) // 不使用
		{
			if (mGnssInsSystem.lastGNSSLCTime > 0 && (mGnssData.timestamp - mGnssInsSystem.lastGNSSLCTime > 360))
			{
				if (mGnssInsSystem.GNSSQUALITYMode >= 4
					&& GNSSPOSmeasR[0] < 0.001  * p_prcopt.GNSSScale
					&& GNSSPOSmeasR[1] < 0.001  * p_prcopt.GNSSScale
					&& GNSSPOSmeasR[2] < 0.001  * p_prcopt.GNSSScale)
				{
					//Position Init
					PositionInitFlag = 1;
			...
			if (p_prcopt.gnssSensorType != 0)
			{
				if (p_prcopt.isUseGNSSVel)
				{
					if (fabs(gnss_speed) < 5 && !mGnssInsSystem.useNativeGNSSPOS)
					{
						if (GNSSPOSmeasR[0] > 0.05  * p_prcopt.GNSSScale
							|| GNSSPOSmeasR[1] > 0.05  * p_prcopt.GNSSScale)
						{
							mGnssInsSystem.GNSSQUALITYMode = 0;
						}
					}
				}
				if (mGnssData.timestamp - mGnssInsSystem.firstGNSSUseTime > 120 && !mGnssInsSystem.useNativeGNSSPOS)
				{
					if ((fabs(GNSSPOSMeasZ[0]) > 6 * sqrt(mUpdataStruct.P_r[0])
						...
						&& (GNSSPOSmeasR[0] > 0.99 * p_prcopt.GNSSScale || GNSSPOSmeasR[1] > 0.99 * p_prcopt.GNSSScale)
						)
					{
						mGnssInsSystem.GNSSflag = 0;
					}
				...
				if (mGnssData.timestamp - mGnssInsSystem.firstGNSSUseTime > 120 && !mGnssInsSystem.useNativeGNSSPOS)
				{
					if ((fabs(GNSSPOSMeasZ[0]) > 50
						...
						&& (GNSSPOSmeasR[0] > 0.99 * p_prcopt.GNSSScale || GNSSPOSmeasR[1] > 0.99 * p_prcopt.GNSSScale) //|| GNSSPOSmeasR[2] > 100)
						)
					{
						mGnssInsSystem.GNSSflag = 0;
					}
				...
				if (GNSSPOSmeasR[2] < 0.0122 * p_prcopt.GNSSScale)
				{
					scaleGNSSfalgAlt = 0;
				}
				if (GNSSPOSmeasR[0] < 0.1 * p_prcopt.GNSSScale
					&& GNSSPOSmeasR[1] < 0.1 * p_prcopt.GNSSScale
					&& GNSSPOSmeasR[2] < 0.1 * p_prcopt.GNSSScale)
				{
					deleteGNSSfalg = 0;
				}
			}
			...
			//	|| (mGnssData.Mode == 4 && sqrt(GNSSPOSmeasR[0] + GNSSPOSmeasR[1]) > 0.2))
		{
			if ((mUpdataStruct.P_r[0] < 9 * GNSSPOSmeasR[0] &&
				...
				|| (mGnssData.timestamp - mGnssInsSystem.lastGNSSLCTime < Tre_gnsslc && mGnssInsSystem.lastNHCLCTime > 0 && mGnssInsSystem.GNSSQUALITYMode != 4))//&& mGnssData.timestamp - mGnssInsSystem.firstGNSSUseTime-mGnssInsSystem.zupttime2 > 1200))
			{
	...
	/*Assumed velocity affects position*/
	if (mGnssInsSystem.GNSSflag == 0 || mGnssInsSystem.GNSSQUALITYMode == 0)
	{
		mGnssInsSystem.GNSSVelflag = 0;
	}

	if (1 == p_prcopt.isUseGNSSZupt)
	{
	...
	{
		if (mGnssInsSystem.GNSSQUALITYMode != 4) mGnssInsSystem.GNSSQUALITYMode = 0;
	}
	...
	if (p_prcopt.isUseGNSSVel && p_prcopt.useGNSSRate == 1 && p_prcopt.isUseNHC) // isUseGNSSVel没使能
	{
	...
	/*
			...
			else if (p_prcopt.isUseNHC && !p_prcopt.isUseOdo)
			{
				if (mGnssData.timestamp - mGnssInsSystem.firstGNSSUseTime > NHCSTARTTIME
					&& (mGnssData.timestamp - mGnssInsSystem.lastVelLCTime > 0.5 || p_prcopt.gnssDataRate == 0)
	...
	if (mGnssInsSystem.GNSSflag == 1
		&& (mGnssInsSystem.GNSSQUALITYMode >= 1 || (mGnssInsSystem.useNativeGNSSPOS == 1 && mGnssInsSystem.GNSSQUALITYMode >= 1))
		&& mGnssData.timestamp - mGnssInsSystem.lastGNSSLCTime > 0.8 / p_prcopt.useGNSSRate)
	{ // GNSS 位置更新
		...
		mGnssInsSystem.lastGNSSLCType = mGnssInsSystem.GNSSQUALITYMode;
		mGnssInsSystem.GNSSflag = 0;
...
/*  ADD origin GNSS data and Updata process
    ...
    分析中
*/
int ADDGNSSDATA(const GnssData mGnssData)
{
	int ret = -1;

	mGnssInsSystem.GNSSQUALITYMode = mGnssData.gpsFixType;
    // 基本的数据判断
	if (mGnssData.week > 1024 && mGnssData.week < 3072)
	{
		if (mGnssInsSystem.firstGNSSTime == -1 && mGnssData.gpsFixType > 0)
		{
	...
	if (p_prcopt.isOnlineMisAlignmentEst) // 安装角在线估计
	{
		if (mGnssInsSystem.mlc_STATUS > INSDATA_NREADY)
		{
			...
			{
				//打印debug 看安装角估计曲线
				writeMISALIGNMsg((int8_t)4, *mGnssInsSystem.RotationCBV, MisAlignmentAiax, misAlignment, &mGnssInsSystem.outputRVB);
			}
			...
			else if (mGnssInsSystem.mlc_STATUS == MISALIGNMENT_ING)
			{
				...
				if (MisAl_status == 2)
				{
					...
					//if (p_prcopt.MisAlignmentAiax[0] != MisAlignmentAiax[0]
					//	|| p_prcopt.MisAlignmentAiax[1] != MisAlignmentAiax[1]
					//	|| p_prcopt.MisAlignmentAiax[2] != MisAlignmentAiax[2])
					//{
					//	InitSoftModeAxix(MisAlignmentAiax); //跟最原始的设置有区别
					//}
					p_prcopt.isUseMisAlignment = 1;
					memcpy(p_prcopt.MisAlignmentAiax, MisAlignmentAiax, 3 * sizeof(int8_t));
					memcpy(p_prcopt.misAlignment, misAlignment, 3 * sizeof(double));

					writeMISALIGNMsg((int8_t)1, *mGnssInsSystem.RotationCBV, p_prcopt.MisAlignmentAiax, p_prcopt.misAlignment, &mGnssInsSystem.outputRVB);
				}
			}
			else
			{
				if (MisAl_status == 2)
				{
						...
						|| p_prcopt.MisAlignmentAiax[2] != MisAlignmentAiax[2])
					{
						...
						memcpy(p_prcopt.MisAlignmentAiax, MisAlignmentAiax, 3 * sizeof(int8_t));
						memcpy(p_prcopt.misAlignment, misAlignment, 3 * sizeof(double));
						writeMISALIGNMsg((int8_t)2, *mGnssInsSystem.RotationCBV, p_prcopt.MisAlignmentAiax, p_prcopt.misAlignment, &mGnssInsSystem.outputRVB);
					}
					// else if (fabs(p_prcopt.misAlignment[0] - misAlignment[0]) > 5 * PI / 180
					// 	|| fabs(p_prcopt.misAlignment[1] - misAlignment[1]) > 5 * PI / 180
					// 	|| fabs(p_prcopt.misAlignment[2] - misAlignment[2]) > 5 * PI / 180)
					// {
					// 	mGnssInsSystem.mErrorType = MisAligmentChange;
					// 	InitHardModeAxix(MisAlignmentAiax);
					// 	ErrorReset(mGnssInsSystem.mErrorType);
					...
					// 	memcpy(p_prcopt.MisAlignmentAiax, MisAlignmentAiax, 3 * sizeof(int8_t));
					// 	memcpy(p_prcopt.misAlignment, misAlignment, 3 * sizeof(double));
					// 	writeMISALIGNMsg((int8_t)3, *mGnssInsSystem.RotationCBV, p_prcopt.MisAlignmentAiax, p_prcopt.misAlignment, &mGnssInsSystem.outputRVB);
					// }
				}
	...
	case INSDATA_READY:
	{
		if (!p_prcopt.isOnlineMisAlignmentEst)
		{
			mGnssInsSystem.mlc_STATUS = MISALIGNMENT_COMPLETE;
		}
	}break;
	...
	case MISALIGNMENT_COMPLETE:
	{
		...
		switch (p_prcopt.initialAttitudeMode) // 姿态初始化模式
		{
		case 0: // 单天线
		{

			int8_t AlignMentflag = MotionAlign(&mGnssInsSystem.premGnssData, &mGnssData, p_prcopt.gnssDataType, &mGnssInsSystem.mNav);
			if (AlignMentflag)
			{
				mGnssInsSystem.mlc_STATUS = ALIGNMENT_COMPLETE;
				mGnssInsSystem.ins_status = INS_ALIGNMENT_COMPLETE;
				mGnssInsSystem.AlignCTime = mGnssData.timestamp; // 记录对准的时间戳
#ifdef DEBUGMESSAGE
		...
		case 1: // 双天线，先不看
		{
			...
			{
				int8_t AlignMentflag = DualantennaAlign(&mGnssInsSystem.premGnssData, &mGnssData, p_prcopt.gnssDataType, &mGnssInsSystem.mNav);
	...
	case INS_FUSING:
	{
		if (p_prcopt.isInsFreInit == 1) // 未开启
		{
			if (mGnssInsSystem.GNSSQUALITYMode > 0)
			{
				if (mGnssInsSystem.lastGNSSLCTime > 0 && mGnssData.timestamp - mGnssInsSystem.lastGNSSLCTime > p_prcopt.insFreTre)
				{
	...
	if (mGnssInsSystem.is_gnss_pos_valid) {
		...
		mGnssInsSystem.PreGNSSQUALITYMode = mGnssInsSystem.GNSSQUALITYMode;
	}
	mGnssInsSystem.GNSSflag = 0;
	ret = 1;
	return ret;
